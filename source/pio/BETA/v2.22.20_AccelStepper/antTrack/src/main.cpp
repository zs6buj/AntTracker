...
//================================================================
  ...
  void buttonEvents()
  {
      ...
      if (phase == set_midfront)
      {
        ...
        {
          adjustButtonActive = true;
          button_millis = millis();
        }
        if (adjustButton.isReleased())
        {
          adjustButtonActive = false;               // wait for async moveTo() to finish
          direction = (direction == CW) ? CCW: CW;  // ternary toggle direction
          s_dir = (direction == CW) ? "CW" : "CCW";
      ...
      if(setButton.isPressed())
      {
        if (phase == set_midfront)
        {
          my_deg = azMidFront;                 // present position is set as 90 angle 
...
// =======================  S E T U P   E E P R O M   =======================
  ...
  */
  ...
  /* We choose not to use the SPI attach methods, but rather pulse pin and direction pin */
    azStepper.setMaxSpeed(max_speed);  // steps/s BEWARE too high value will stall the stepper and blow the TB6600 
    azStepper.setAcceleration(acceleration); // acceleration rate in steps/s/s
   // azStepper.setMaxSpeed(max_speed);        // steps/s - 3200 == 60RPM, 1rp/s
   // azStepper.setAcceleration(acceleration); // acceleration rate in steps/s/s
    elStepper.setMaxSpeed(max_speed);        // steps/s - 3200 == 60RPM, 1rp/s
    elStepper.setAcceleration(acceleration); // acceleration rate in steps/s/s
    /*
    ...
    */ 
    phase = set_midfront;
    log.printf("Stepper max speed:%u  acceleration:%1.1f  gear_ratio:1/%u\n", max_speed, acceleration, gear_ratio);
  #endif
