...
//================================================================================================= 
#if (MEDIUM_IN == 2) && (WIFI_PROTOCOL == 2)     //  UDP
  bool sendMavUDP(mavlink_message_t* msgptr) 
  {
...
//================================================================================================= 
#if (MEDIUM_IN == 3)     //  BT
bool sendMavBT(mavlink_message_t* msgptr) {
  bool msgSent = false;
  uint16_t len = mavlink_msg_to_send_buffer(sendbuf, msgptr);
...
bool readMavBT(mavlink_message_t* msgptr)  
{  
    bool msgRcvd = false;
    mavlink_status_t _status;
...
{ 
  ...
  gotRecord = false;
    ...
    { 
      #if defined DEBUG_MAV_INPUT
          log.printf("Mav message ID: #%2u read\n", msg.msgid);
        #if defined DEBUG_MAV_BUFFER
            printMav
        #endif
      #endif
      switch(msg.msgid) 
      {
        ...
        case MAVLINK_MSG_ID_SYS_STATUS:   // #1
          ...
          hud_bat1_volts = ap_voltage_battery1 * 0.01F;         // mV -> dV
          hud_bat1_amps = ap_current_battery1 * 0.1F;           // cA -> dA
        ...
        case MAVLINK_MSG_ID_GPS_RAW_INT:          // #24
          ...
          if (ap24_sat_visible > 15) {                // @rotorman 2021/01/18
            if (ap24_sat_visible == 255)
              hud_num_sats = 0; // for special case 255 == unknown satellite count
            else
             hud_num_sats = 15; // limit to 15 due to only 4 bits available
          }  else 
              hud_num_sats = ap24_sat_visible;
                          
          gpsfixGood = (ap24_fixtype>=3);
        ...
        case MAVLINK_MSG_ID_RC_CHANNELS_RAW:         // #35
          ...
          if ((!rssi65) && (!rssi109)) { // If no #65 and no #109 received, then use #35
            ...
            hud_rssi = ap_rssi; 
             #if defined MAV_DEBUG_ALL || defined DEBUG_RSSI
              log.print("Mavlink from FC #35 RC_CHANNELS_RAW: ");
              log.print("  RSSI: "); log.println(ap_rssi);                // %
            #endif              
          } 
          break;
        ...
        case MAVLINK_MSG_ID_RADIO_STATUS:         // #109
          ...
          hud_rssi = ap_rssi;  
          #if defined MAV_DEBUG_ALL || defined DEBUG_RSSI
            log.print("Mavlink from FC #109 RADIO_STATS: ");
            log.print("  RSSI: "); log.println(ap_rssi);                // %
          #endif               
          break;    
