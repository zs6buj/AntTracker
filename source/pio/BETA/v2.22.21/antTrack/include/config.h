...
//#define ESP32_VARIANT     9    // ESP32-C3 Supermini
...
//=============================================================================================
  ...
  //#define AZ_SERVO_360   // Means the azimuth servo can point in a 360 deg circle, elevation servo 90 deg
    ...
    // for testing 270 deg = left, 0 deg = straight ahead, 90 deg = right, 180 deg = behind
  #endif 
  #if (defined SERVOS)                   
    #define azMidFront  90  // 0 deg = left, 90 deg = straight ahead, 180 deg = right, 270 deg = behind
  #endif
  ...
  typedef enum direction_set {none= 0, CW = 1, CCW = -1 } direction_t;
    ...
    const float steps_per_degree = (float)max_steps / (float)360;
    bool  adjustButtonActive = false;
    const uint16_t max_speed = 1000;           // steps/s - 6400 == 60RPM, 1rp/s
    const uint16_t acceleration = 2000;        // steps/s/s
    const uint16_t gear_ratio = 10;            // inverse. So 10 means 1/10. Gear ratio of motor gearbox. 1 if no gearing
    std::string s_dir; 
  #endif
...
//============================================================================================= 
  ...
  //=================================================================================================  
      ...
      #include <Servo.h>           
        ...
        #include <AccelStepper.h>  
      #else
        #include <Servo.h> // ask Copilot about ESP8266_ISR_Servo library
      #endif          
    #endif
  //=================================================================================================   
