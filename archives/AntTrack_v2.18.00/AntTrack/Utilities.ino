    ...
    void DisplayFlightInfo() {
      ...
      if (display_mode != flight_info) {
        ...
        if (millis() - info_millis > 2000) {    // refresh rate
          ...
          yy = 0 * char_h_px;      
          display.setCursor(xx, yy);            
          snprintf(snprintf_buf, snp_max, "Sats %d RSSI %ld%%", pt_numsats, pt_rssi); 
          ...
          #endif  
          display.setCursor(xx, yy);            
          snprintf(snprintf_buf, snp_max, "Alt %u  Hdg %3.0f", (int)roundf(pt_home_alt * 0.1F), cur.hdg); 
          display.fillRect(xx+(4*char_w_px), yy, (3 * char_w_px), char_h_px, SCR_BACKGROUND);  // blank Alt
