...
v2.05 2019/03/17 UNTESTED BETA Support for restoring home settings from EEPROM after power interruption  
...
void CheckForTimeouts() {
  uint32_t cMillis = millis();
    if ((gpsGood) && (cMillis - gps_millis >= 5000)){
      gpsGood = false;   // If no GPS packet for 5 seconds then GPS timeout 
      telGood = 0;
