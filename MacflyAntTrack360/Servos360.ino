...
void  PositionServos(float Az, float El, float hmHdg) {
  ...
  int MinPWM = 650;
  int MaxPWM = 2400;
  int MinPWM_tilt = 1125;
  int MaxPWM_tilt = 1875;
...
/*  
  ...
  elPWM = map(LastGoodEl, ulEl, llEl, MinPWM_tilt, MaxPWM_tilt);   // Servos happen to be mounted such that action is reversed
*/
  // new version for 360 degrees : Note my 180 degree servos have a PWM range of 650 through 2400 microseconds - centre 1525
  azPWM = map(pntAz, ulAz, llAz, MinPWM, MaxPWM);  // Map the az / el to the servo PWMs
  elPWM = map(El, ulEl, llEl, MinPWM_tilt, MaxPWM_tilt);     // Servos happen to be mounted such that action is reversed
