...
/*================================================================================================= 
  ...
  By downloading this software you are agreeing to the terms specified in this page and the spirit of thereof.
   ...
   Information and ideas were obtained from GhettoProxy by Guillaume S.
   ...
   supports an OLED display. The ESP board will support UART, WiFi, Bluetooth and BLE telemetry
  input.
...
#include "debug.h"
  ...
  CRSF   crsf;            // instantiate crsf object
    ...
    typedef enum phase_set {set_midfield = 0, set_home = 1, set_done = 3} phase_t;
    phase_t phase;
  ...
  void setupEEPROM(); 
  void buttonEvents();
  void moveMotors(uint16_t, uint16_t);
  void pointMotors(int16_t, int16_t, int16_t);
  ...
  #endif
  void displayHeadingSource(uint8_t);
  void testMotors();
  void firstStoreHome();
  void finalStoreHome();
  ...
  void buttonEvents()
  {
      ...
      if(setButton.isPressed())
      {
        if (phase == set_midfield)
        {
          ...
          std::cout << "Mid-field set" << std::endl; 
          phase = set_home;  
        } 
      } 
    #endif  
...
void setup() 
{
  ...
  // =======================  S E T U P   B U T T O N S   =======================
    ...
    upButton.setDebounceTime(50); // mS
    ezButton dnButton(Pdn);
    dnButton.setDebounceTime(50); 
  #endif   
  ezButton setButton(setPin);
  setButton.setDebounceTime(50); // mS
  #if defined STEPPERS
    ezButton adjustButton(adjustPin);
    adjustButton.setDebounceTime(50);  
  #endif  
  // ======================== Setup I2C ==============================
  #if (( defined ESP32 ) || (defined ESP8266) )
  ...
  */
  ...
  // ====================== SETUP SERVOS or STEPPERS  ===========================
    ...
    //elStepper.attachEnable(elEnaPin, 200, LOW );    
    phase = set_midfield;
  #endif

  moveMotors(azStart, elStart);   // Move Motors to "start position
    ...
    if (finalHomeStored) 
    {
      ...
      {  //  every time there is new GPS data 
        ...
        if ( (hc_vector.dist >= minDist) || ((int)cur.alt_ag >= minAltAg) ) 
            pointMotors((uint16_t)hc_vector.az, (uint16_t)hc_vector.el, (uint16_t)hom.hdg);  // Motors pointed relative to "home midfield" heading >>>>>>
        new_GPS_data = false;
      }
    }
