  ...
  typedef enum phase_set {startup = 0, set_midfront = 1, set_home = 2, set_done = 3} phase_t;
  phase_t phase = startup;
...
//     I N S T A N T I A T E    M O T O R    O B J E C T 

#if defined _PWMSERVO  
  PWMServo azServo;             // Azimuth
  PWMServo elServo;             // Elevation  
#elif defined _MOBASTEP  
  MoToStepper azStepper(mbstStepRev,  STEPDIR); 
  MoToStepper elStepper(mbstStepRev,  STEPDIR);  
#elif defined _MOBASERVO  // ESP32 and ESP8266
  MoToServo azServo; 
  MoToServo elServo;   
#elif defined _ESPSERVO  // not Cx and Sx
  Servo azServo; 
  Servo elServo;   
#elif defined _ACCELSTEP 
  AccelStepper azStepper(AccelStepper::DRIVER, azPulsePin,  azDirPin);  // DRIVER see AccelStepper.h
  AccelStepper elStepper(AccelStepper::DRIVER, elPulsePin,  elDirPin);   
#elif defined _8266SERVO 
  // not used right now
#endif
