...
// loop until char pattern found
  chr = NxtChar();  // wait for the first char
  uint8_t found = 0;
  while (found == 0) 
  {   
    if (chr==0xFE) {   
    ...
    */
    if ( ( (ch[1] == 0x0D) || (ch[1] == 0x18)|| (ch[1] == 0x23) ) && (ch[2] == 0xF1)  )  {  // fp2 OTA     
      FPort2++; 
    ...
    if ( (ch[1] == 0x08) && ( (ch[2] >= 0) && (ch[2] < 0x1b) ) )   {     // fp2 downlink  
      FPort2++; 
