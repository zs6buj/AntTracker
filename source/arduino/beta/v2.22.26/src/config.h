...
//=============================================================================================
  ...
  #endif
  #if defined SERVOS
    #define SERVO_SPEED     80   // 5=very slow, 0=no effect
  /* MobaTools QUOTE
  ...
  */
  ...
  // motor movement is constrained by these values
    ...
    const int16_t minEl = 0;          // El lower limit in degrees, horizontal 
    const int16_t maxEl = 90;         // El upper limit in degrees, straight up
...
//=========================================================================   
  ...
  */  
  ...
  //========================================================================= 
    ...
    */
    int8_t in_rxPin =         D3;  // SoftwareSerial 1
    #define in_txPin          D4   // for mavlink
    #if (HEADINGSOURCE == 4)  // Tracker box GPS 
      int8_t boxgps_rxPin =     D5;  // alt D8 SoftwareSerial 2 for tracker box GPS if applicable
      #define boxgps_txPin      D6   // alt D9
    #endif
    bool in_invert = false;         // ONLY FOR FrSky S.Port, NOT F.Port, NOT MAVLINK
    #define set_Pin           D5   // LOW == pushed    
    #define StatusLed         D6   // Off=No good GPS yet, flashing=good GPS but home not set yet, solid = ready to track
    #define BuiltinLed        -1   // none, but must be defined!
                                   // Also TXD1 - Serial-1 debug log out SHARED WITH BOARD LED  
    //#define INVERT_LED             // NodeMCU LED action is inverse                  
    #if defined SERVOS
      #define azPWM_Pin       D7  // azimuth servo
      #define elPWM_Pin       D8  // connected to flash elevation servo
    #endif
    #if defined STEPPERS
      #define adjustPin       D10  // white    
      #define azPulsePin      D7  // orange
      #define azDirPin        D8  // grey
    ...
    #define SSD1306_DISPLAY         // OLED display type    
      ...
      #define SDA           D1        // D4 I2C OLED D4board and/or Compass
      #define SCL           D2        // D5 I2C OLED board and/or Compass
    #endif  
  #endif
#endif
//============================================================================================= 
