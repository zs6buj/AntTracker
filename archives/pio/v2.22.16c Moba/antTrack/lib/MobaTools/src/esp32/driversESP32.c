...
// zs6buj There are generally max 16 pwm ouputs, but the C & S series only have 8. These are used by servos and softleds
pwmUse_t pwmUse[MAX_PWM];

int8_t _findPwmNbr( ) {
    // find a free pwm Nbr and return it
    // value is negative if no free channel available
    int8_t pwmNbr;
    for( pwmNbr=(MAX_PWM-1); pwmNbr>=0; pwmNbr-- ) {
...
int8_t freePwmNbr( uint8_t pwmNbr ) {
    if ( pwmNbr > (MAX_PWM-1) ) return false;
    setPwmDuty( pwmNbr, 0 );
...
// zs6buj    
    ...
    if(!(DPORT_GET_PERI_REG_MASK(SYSTEM_PERIP_CLK_EN0_REG, SYSTEM_PERIP_CLK_EN0_REG)) ||
       (DPORT_GET_PERI_REG_MASK(SYSTEM_PERIP_RST_EN0_REG, SYSTEM_LEDC_RST) ) )  {
        // LEC HW in reset or no clock enabled
        DPORT_SET_PERI_REG_MASK(SYSTEM_PERIP_CLK_EN0_REG, SYSTEM_PERIP_CLK_EN0_REG);
        DPORT_CLEAR_PERI_REG_MASK(SYSTEM_PERIP_RST_EN0_REG, SYSTEM_LEDC_RST);        
#else
...
//#if CONFIG_IDF_TARGET_ESP32S2 || CONFIG_IDF_TARGET_ESP32S3 || CONFIG_IDF_TARGET_ESP32C3
  #if ESP32_S_OR_C
    // zs6buj For the ESP32-S or C series, the LEDC peripheral only supports low-speed mode
    pinMatrixOutAttach(pin, LEDC_LS_SIG_OUT0_IDX + channel, false, false);
#else    
