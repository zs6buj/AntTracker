...
uint8_t GetProtocol() { 
      ...
      DisplayPrintln("Telem found at "+ s_baud);   
   #else 
     baud = 57600;  // default if not autobaud
     Debug.print("Default baud rate is ");  Serial.print(baud); Serial.println(" b/s"); 
     String s_baud=String(baud);   // integer to string. "String" overloaded
     DisplayPrintln("Default baud is "+ s_baud);  
   #endif  
   DetectProtocol(baud);
...
uint32_t GetBaud(uint8_t rxPin) {
  Debug.print("AutoBaud - Sensing rxPin "); Debug.println(rxPin);
 // Debug.printf("AutoBaud - Sensing rxPin %2d \n", rxPin );
  uint8_t i = 0;
  uint8_t col = 0;
  pinMode(rxPin, INPUT);       
  ...
  while (gb_baud == 0) {
    ...
    if (col > 60) {
      Debug.println(); 
        Debug.print("No telemetry found on pin "); Debug.println(rxPin);
